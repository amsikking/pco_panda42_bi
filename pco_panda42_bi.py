...
class Camera:
    '''
    Basic device adaptor for PCO.panda 4.2 bi USB sCMOS camera. Many more
    ...
    def __init__(self,
        ...
        assert camera_name.value == b'pco.panda 4.2 bi'
    ...
    def _set_pixel_rate(self, rate):
        ...
        assert rate == 46000000, "rate '%s' not allowed"%rate
        dll.set_pixel_rate(self.handle, rate)
        assert self._get_pixel_rate() == rate
        if rate == 46000000:  self.line_time_us = 12.174
        if self.very_verbose:
...
def legalize_image_size(
    height_px='max', width_px='max', name='PCO.panda4.2 bi', verbose=True):
